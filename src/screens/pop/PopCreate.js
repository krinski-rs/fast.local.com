...
class PopCreate extends Component {
	constructor(props) {
		super(props);		
		this.state = {
			open: false,
	...
	save(event){
    	...
    	}, "http://fast.api.local.com/api/pop/", null);
	    retorno.then((obj)=>{
		    if(obj.ok){
		    	obj.json().then((json)=>{
    				this.setState(prevState => ({
    					error: false,
    					open:true,
    					mensagem: "Pop id '"+json.id+"' cadastrado com sucesso!!"
    				}));
		    	});
		    }else{
		    	obj.json().then((json)=>{
...
export default withStyles(styles)(PopCreate);
